...
#else
    char      *buffer;		/* data to send, malloc-ed */
#endif
    /*
...
#else /* to avoid name clash with macros defined in lwip's sockets.h */
    int       (*accept_fn)(struct iperf_test *);
    int       (*listen_fn)(struct iperf_test *);
    int       (*connect_fn)(struct iperf_test *);
    int       (*send_fn)(struct iperf_stream *);
    int       (*recv_fn)(struct iperf_stream *);
    int       (*init_fn)(struct iperf_test *);
#endif
...
#if IPERF_LOCK_ENABLE
static os_mutex_handle_t iperf_mutex = NULL;

#define IPERF_MUTEX_INIT()     do { if(!iperf_mutex) \
                                        iperf_mutex = os_mutex_create(); \
                               }while(0)
#define IPERF_MUTEX_LOCK()     do { \
                                    if(iperf_mutex) \
                                        os_mutex_lock(iperf_mutex, WAIT_FOREVER);\
                               }while(0)
#define IPERF_MUTEX_UNLOCK()     do { \
                                     if(iperf_mutex) \
                                        os_mutex_unlock(iperf_mutex);\
                               }while(0)
#define IPERF_MUTEX_DEINIT()   do { \
                                    if(iperf_mutex) \
                                        os_mutex_destroy(iperf_mutex); \
                               }while(0)
#else
#define IPERF_MUTEX_INIT()     
#define IPERF_MUTEX_LOCK()     
#define IPERF_MUTEX_UNLOCK()
#define IPERF_MUTEX_DEINIT()
#endif
