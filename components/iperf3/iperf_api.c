...
usage_long(void)
{
    //printf(usage_longstr, UDP_RATE / (1024*1024), DURATION, DEFAULT_TCP_BLKSIZE / 1024, DEFAULT_UDP_BLKSIZE);
    printf("usage ex:\nserver:iperf -s -4\nudp client:iperf -c 192.168.31.183 -t 10 -u\ntcp client:iperf -c 192.168.31.183 -t 10\n");
}
#endif
...
iperf_on_connect(struct iperf_test *test)
{
    ...
    now_secs = now.tv_sec;
    if (test->json_output)
	cJSON_AddItemToObject(test->json_start, "timestamp", iperf_json_printf("time: %s  timesecs: %d", "(N.A)", (int64_t) now_secs));
    else if (test->verbose)
	iperf_printf(test, report_time, "(N.A)");
#endif
	...
	if (test->json_output)
        ...
        len = sizeof(sa);
        getsockname(test->ctrl_sck, (struct sockaddr *)&sa, &len);
	    ...
	    iperf_printf(test, report_connecting, test->server_hostname, port, test->server_port);
	    if (test->reverse)
...
#else
    if (test->protocol->init_fn) {
        if (test->protocol->init_fn(test) < 0)
#endif
            return -1;
    }
...
#endif		
    if (test->settings->rate && test->json_start)
        cJSON_AddNumberToObject(test->json_start, "target_bitrate", test->settings->rate);
	cJSON_Delete(j);
    }
	...
	if (j_cpu_util_total == NULL || j_cpu_util_user == NULL || j_cpu_util_system == NULL || j_sender_has_retransmits == NULL) {
		...
		for (i=0; i<n; ++i) {
			...
			if (j_id == NULL || j_bytes == NULL || j_retransmits == NULL || j_jitter == NULL || j_errors == NULL || j_packets == NULL) {
			    ...
			    sid = j_id->valueint;
			    bytes_transferred = j_bytes->valuedouble;
...
iperf_defaults(struct iperf_test *testp)
{
    ...
    tcp->send_fn = iperf_tcp_send;
    tcp->recv_fn = iperf_tcp_recv;
    tcp->init_fn = NULL;
#endif
    ...
    udp->send_fn = iperf_udp_send;
    udp->recv_fn = iperf_udp_recv;
    udp->init_fn = iperf_udp_init;
#endif
    ...
    sctp->send_fn = iperf_sctp_send;
    sctp->recv_fn = iperf_sctp_recv;
    sctp->init_fn = iperf_sctp_init;
#endif
...
iperf_stats_callback(struct iperf_test *test)
{
	...
	irp = TAILQ_LAST(&rp->interval_results, irlisthead);
        ...
        else /* or use timestamp from beginning */
        {
            ...
            rp->end_time = rp->start_time;
        }
        /* now save time of end of this interval */
        iperf_time_add_usecs(&rp->end_time, SEC_TO_US);
	...
	/* Cant' find any reason to keep all interval results */
	{
            ...
            for (irp = TAILQ_FIRST(&rp->interval_results); irp != NULL; irp = nirp) {
                nirp = TAILQ_NEXT(irp, irlistentries);
                free(irp);
            }
            TAILQ_INIT(&rp->interval_results);
	}
#endif
...
#else
        if (test->settings->blksize > 4096) {
            test->settings->blksize = 4096; //8K
        } else if (test->settings->blksize < 0) {
            printf("eror------%s:%d------%d----\n", __func__, __LINE__, test->settings->blksize);
            return NULL;
        }
...
iperf_printf(struct iperf_test *test, const char* format, ...)
{
	...
	if (test->title)
	    printf("%s:  ", test->title);
	va_start(argp, format);
	os_vprintf(LM_APP, LL_INFO, format, argp);
	va_end(argp);
#endif
    }
    else if (test->role == 's') {
