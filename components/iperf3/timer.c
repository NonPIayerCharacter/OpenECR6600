...
static void list_add(void *handle, Timer *t)
{
    ...
    if (test->timers == NULL ) {
        ...
        if (iperf_time_compare(&t->time, &test->timers->time) < 0) {
...
static void list_remove(void *handle, Timer *t)
{
    struct iperf_test *test = (struct iperf_test *)handle;
...
static void list_resort(void *handle, Timer *t)
{
    struct iperf_test *test = (struct iperf_test *)handle;
