...
Nwrite(int fd, const char *buf, size_t count, int prot)
{
    register ssize_t r;
    register size_t nleft = count;
    long long time_inms = os_time_get();
    long long time_oums = os_time_get();
	...
	if (r < 0) {
	    switch (errno) {
	    case ENOMEM:
	        time_oums = os_time_get();
            if (time_oums - time_inms > 5000) {
                time_inms = os_time_get();
...
#else
	if ((flags = fcntl(fd, F_SETFL, newflags)) < 0) {
	    printf("fcntl(F_SETFL)%d,%d, %x\n", fd, flags, newflags);
#endif
	    return -1;
	}
    return 0;
