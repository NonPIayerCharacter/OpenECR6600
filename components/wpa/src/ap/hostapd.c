...
static void hostapd_reload_bss(struct hostapd_data *hapd)
{
	...
	if (hostapd_setup_wpa_psk(hapd->conf)) {
		wpa_printf_error(MSG_ERROR, "Failed to re-configure WPA PSK "
    ...
    */
	...
	if ((hapd->conf->wpa/* || hapd->conf->osen*/) && hapd->wpa_auth == NULL) {
		...
		if (hostapd_set_generic_elem(hapd, wpa_ie, wpa_ie_len))
			wpa_printf_error(MSG_ERROR, "Failed to configure WPA IE for "
				   "the kernel driver.");
	...
	if (hapd->conf->ssid.ssid_set &&
		...
		wpa_printf_error(MSG_ERROR, "Could not set SSID for kernel driver");
		/* try to continue */
	}
...
static int hostapd_broadcast_wep_set(struct hostapd_data *hapd)
{
	...
	if (ssid->wep.default_len &&
	    hostapd_drv_set_key(hapd->conf->iface,
				...
				ssid->wep.len[idx])) {
		wpa_printf_warning(MSG_WARNING, "Could not set WEP encryption.");
		errors++;
	}
...
static void hostapd_free_hapd_data(struct hostapd_data *hapd)
{
	...
	if (!hapd->started) {
		wpa_printf_error(MSG_ERROR, "%s: Interface %s wasn't started",
...
static int hostapd_setup_encryption(char *iface, struct hostapd_data *hapd)
{
	...
	for (i = 0; i < 4; i++) {
		if (hapd->conf->ssid.wep.key[i] &&
			...
			wpa_printf_warning(MSG_WARNING, "Could not set WEP "
				   "encryption.");
			return -1;
		}
		if (hapd->conf->ssid.wep.key[i] &&
...
static int hostapd_validate_bssid_configuration(struct hostapd_iface *iface)
{
	...
	if (bits > 40) {
		wpa_printf_error(MSG_ERROR, "Too many bits in the BSSID mask (%u)",
			   bits);
		return -1;
	}
...
skip_mask_ext:
	...
	for (i = 0; i < ETH_ALEN; i++) {
		if ((hapd->own_addr[i] & mask[i]) != hapd->own_addr[i]) {
			wpa_printf_error(MSG_ERROR, "Invalid BSSID mask " MACSTR
				   " for start address " MACSTR ".",
				   MAC2STR(mask), MAC2STR(hapd->own_addr));
			wpa_printf_error(MSG_ERROR, "Start address must be the "
...
static int hostapd_setup_bss(struct hostapd_data *hapd, int first)
{
	...
	if (conf->tnc && tncs_global_init() < 0) {
		wpa_printf_error(MSG_ERROR, "Failed to initialize TNCS");
		return -1;
	}
#endif /* EAP_SERVER_TNC */

	if (hapd->started) {
		wpa_printf_error(MSG_ERROR, "%s: Interface %s was already started",
			   __func__, conf->iface);
		return -1;
	}
	hapd->started = 1;

#if 0 //we can't get here.
	if (!first || first == -1) {
		...
		if (!is_zero_ether_addr(conf->bssid)) {
			...
			if (hostapd_mac_comp(hapd->own_addr,
			    ...
			    0) {
				wpa_printf_error(MSG_ERROR, "BSS '%s' may not have "
		...
		if (hostapd_if_add(hapd->iface->bss[0], WPA_IF_AP_BSS,
				   ...
				   NULL,
				   first == -1)) {
			wpa_printf_error(MSG_ERROR, "Failed to add BSS (BSSID="
	...
	if (ssid_len < 0) {
		wpa_printf_error(MSG_ERROR, "Could not read SSID from system");
		return -1;
	}
	...
	if (hostapd_setup_wpa_psk(conf)) {
		wpa_printf_error(MSG_ERROR, "WPA-PSK setup failed.");
		return -1;
	}
	...
	if (hapd->radius == NULL) {
		wpa_printf_error(MSG_ERROR, "RADIUS client initialization failed.");
		return -1;
	}
	...
	if (conf->radius_das_port) {
		...
		if (hapd->radius_das == NULL) {
			wpa_printf_error(MSG_ERROR, "RADIUS DAS initialization "
				   "failed.");
			return -1;
		}
	}
	...
	//if (ieee802_1x_init(hapd)) {
		//wpa_printf_error(MSG_ERROR, "IEEE 802.1X initialization failed.");
		//return -1;
	//}
	...
	if (gas_serv_init(hapd)) {
		wpa_printf_error(MSG_ERROR, "GAS server initialization failed");
		return -1;
	}
	...
	if (conf->qos_map_set_len &&
		...
		wpa_printf_error(MSG_ERROR, "Failed to initialize QoS Map");
		return -1;
	}
#endif /* CONFIG_INTERWORKING */

	//if (conf->bss_load_update_period && bss_load_update_init(hapd)) {
	//	wpa_printf_error(MSG_ERROR, "BSS Load initialization failed");
	//	return -1;
	//}
...
static int setup_interface(struct hostapd_iface *iface)
{
	 ...
	 * driver interface.
	 */
#if 0 // not need do this, because we only have one bss	 
	...
	if (hapd->iconf->country[0] && hapd->iconf->country[1]) {
        ...
        memcpy(hapd->iconf->country, country, 2);
        hapd->iconf->country[2] = ' ';
#endif
...
static int setup_interface2(struct hostapd_iface *iface)
{
	...
	if (hostapd_get_hw_features(iface)) {
		...
		if (ret < 0) {
			wpa_printf_error(MSG_ERROR, "Could not select hw_mode and "
				   "channel. (%d)", ret);
			goto fail;
		}
		if (ret == 1) {
...
static void fst_hostapd_set_ies_cb(void *ctx, const struct wpabuf *fst_ies)
{
	...
	if (hapd->iface->fst_ies != fst_ies) {
		...
		if (ieee802_11_set_beacon(hapd))
			wpa_printf_warning(MSG_WARNING, "FST: Cannot set beacon");
	}
}
						 ...
						 int err)
{
	...
	if (iface->conf->channel) {
		...
		/*if (!delay_apply_cfg &&
		    hostapd_set_freq(hapd, hapd->iconf->hw_mode, iface->freq,
				     ...
				     hapd->iconf->vht_oper_centr_freq_seg0_idx,
				     hapd->iconf->vht_oper_centr_freq_seg1_idx)) {
			wpa_printf_error(MSG_ERROR, "Could not set channel for "
				   "kernel driver");
			goto fail;
		}*/
	}
	...
	if (iface->current_mode) {
		if (hostapd_prepare_rates(iface, iface->current_mode)) {
			wpa_printf_error(MSG_ERROR, "Failed to prepare rates "
				   "table.");
	...
	if (hapd->iconf->rts_threshold > -1 &&
		...
		wpa_printf_error(MSG_ERROR, "Could not set RTS threshold for "
			   "kernel driver");
		goto fail;
	}
	...
	if (hapd->iconf->fragm_threshold > -1 &&
		...
		wpa_printf_error(MSG_ERROR, "Could not set fragmentation threshold "
			   "for kernel driver");
		goto fail;
	}
    #endif
	...
	/*if (hostapd_driver_commit(hapd) < 0) {
		wpa_printf_error(MSG_ERROR, "%s: Failed to commit driver "
	...
	if (hapd->iconf->fst_cfg.group_id[0]) {
		...
		if (!iface->fst) {
			wpa_printf_error(MSG_ERROR, "Could not attach to FST %s",
...
fail:
	wpa_printf_error(MSG_ERROR, "Interface initialization failed");
	hostapd_set_state(iface, HAPD_IFACE_DISABLED);
...
int hostapd_setup_interface(struct hostapd_iface *iface)
{
	...
	if (ret) {
		wpa_printf_error(MSG_ERROR, "%s: Unable to setup interface.",
			   iface->bss[0]->conf->iface);
		return -1;
	}
...
fail:
	wpa_printf_error(MSG_ERROR, "Failed to set up interface with %s",
		   config_file);
	if (conf)
			   ...
			   const char *config_fname, int debug)
{
	...
	if (iface) {
		...
		if (conf == NULL)
			return NULL;
		if (conf->num_bss > 1) {
			wpa_printf_error(MSG_ERROR, "Multiple BSSes specified in BSS-config");
			hostapd_config_free(conf);
			return NULL;
		}
	...
	if (iface->conf->bss[bss_idx]->iface[0] == '\0' &&
		...
		wpa_printf_error(MSG_ERROR, "Interface name not specified in %s",
			   config_fname);
		if (new_iface)
...
int hostapd_enable_iface(struct hostapd_iface *hapd_iface)
{
	...
	if (hapd_iface->bss[0]->drv_priv != NULL) {
		wpa_printf_error(MSG_ERROR, "Interface %s already enabled",
...
int hostapd_reload_iface(struct hostapd_iface *hapd_iface)
{
	...
	for (j = 0; j < hapd_iface->num_bss; j++)
		hostapd_set_security_params(hapd_iface->conf->bss[j], 1);
	if (hostapd_config_check(hapd_iface->conf, 1) < 0) {
		wpa_printf_error(MSG_ERROR, "Updated configuration is invalid");
		return -1;
	}
...
hostapd_iface_alloc(struct hapd_interfaces *interfaces)
{
	...
	hapd_iface = interfaces->iface[interfaces->count] =
		hostapd_alloc_iface();
	if (hapd_iface == NULL) {
		wpa_printf_error(MSG_ERROR, "%s: Failed to allocate memory for "
		     ...
		     const char *ctrl_iface, const char *driver)
{
	...
	if (conf->driver == NULL) {
		wpa_printf_error(MSG_ERROR, "No driver wrappers registered!");
		hostapd_config_free(conf);
		return NULL;
	}
...
int hostapd_add_iface(struct hapd_interfaces *interfaces, char *buf)
{
	...
	*ptr++ = '\0';
	...
	if (hapd_iface == NULL) {
		wpa_printf_error(MSG_ERROR, "%s: Failed to allocate memory "
...
{
	struct wpabuf *beacon_extra = NULL, *proberesp_extra = NULL, *assocresp_extra = NULL;
				...
				const struct hostapd_freq_params *freq_params)
{
	...
	case 160:
		...
		break;
	default:
		wpa_printf_warning(MSG_WARNING, "Unknown CSA bandwidth: %d",
