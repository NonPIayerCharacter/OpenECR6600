				...
				int max_len)
{
	...
	*pos++ = WLAN_EID_COUNTRY;
	*pos++ = 6; /* length will be set later */
	os_memcpy(pos, hapd->iconf->country, 3); /* e.g., 'US ' */
	pos += 3;

    mode = hapd->iface->current_mode;
    *pos++ = mode->channels[0].chan;
    *pos++ = mode->num_channels;
    *pos++ = 0x14; //max tx power
...
#define MAX_PROBERESP_LEN 768
	...
	if (hapd->iconf->ieee80211ax && !disable_ht) {
		      ...
		      int ssi_signal)
{
    ...
    //wpa_printf(MSG_DEBUG, "send probe response");
					...
					size_t *resp_len)
{
	...
	if (hapd->conf->wps_state && hapd->wps_probe_resp_ie &&
		...
		wpa_printf_warning(MSG_WARNING, "Device is trying to offload WPS "
	...
	if ((hapd->conf->p2p & P2P_ENABLED) && hapd->p2p_probe_resp_ie &&
		...
		wpa_printf_warning(MSG_WARNING, "Device is trying to offload P2P "
			   "Probe Response while not supporting this");
#endif  /* CONFIG_P2P */
...
#define BEACON_TAIL_BUF_SIZE 512
	...
	if (head == NULL || tail == NULL) {
		wpa_printf_error(MSG_ERROR, "Failed to set beacon data");
		os_free(head);
		os_free(tail);
		return -1;
	}
...
int ieee802_11_set_beacon(struct hostapd_data *hapd)
{
	...
	if (res)
		wpa_printf_error(MSG_ERROR, "Failed to set beacon parameters");
	else
		ret = 0;
//fail:
