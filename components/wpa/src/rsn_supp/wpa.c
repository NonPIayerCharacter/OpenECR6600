...
void wpa_sm_key_request(struct wpa_sm *sm, int error, int pairwise)
{
	...
	wpa_set_eth_eapol_enc(ETH_EAPOL_ENC_DISABLE);
			       ...
			       struct wpa_eapol_ie_parse *ie)
{
	...
	if (ie->igtk) {
		...
		if (keyidx > 4095) {
			wpa_msg(sm->ctx->msg_ctx, MSG_WARNING,
				"WPA: IGTK KeyID %d > 4095", keyidx);
			//eswin modify for logic is the same as the FW
			keyidx = igtk->keyid[0];
			//return -1;
		}
				      ...
				      int ver, u16 key_info)
{
	...
	wpa_set_eth_eapol_enc(ETH_EAPOL_ENC_ENABLE);
		    ...
		    const u8 *buf, size_t len)
{
	...
	if (key_info & WPA_KEY_INFO_KEY_TYPE) {
        ...
        if (key_info & WPA_KEY_INFO_MIC) {
			/* 3/4 4-Way Handshake */
			wpa_printf(MSG_DEBUG, "[time]<wpa m4>");
			wpa_supplicant_process_3_of_4(sm, key, ver, key_data,
						      key_data_len);
			wpa_printf(MSG_DEBUG, "[time]</wpa m4>");
		} else {
			/* 1/4 4-Way Handshake */
			wpa_printf(MSG_DEBUG, "[time]<wpa m2>");
