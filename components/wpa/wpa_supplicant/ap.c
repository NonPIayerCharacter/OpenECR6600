			      ...
			      struct hostapd_config *conf)
{
	...
	if (conf->hw_mode == NUM_HOSTAPD_MODES) {
		wpa_printf_error(MSG_ERROR, "Unsupported AP mode frequency: %d MHz",
			   ssid->frequency);
		return -1;
	}
				  ...
				  struct hostapd_config *conf)
{
...
#if 0
	if (ssid->pbss > 1) {
		wpa_printf_error(MSG_ERROR, "Invalid pbss value(%d) for AP mode",
			   ssid->pbss);
		return -1;
	}
	bss->pbss = ssid->pbss;
	...
	if (ssid->ssid_len == 0) {
		wpa_printf_error(MSG_ERROR, "No SSID configured for AP mode");
		return -1;
	}
    ...
    if (wpa_s->ap_iface->bss && wpa_s->ap_iface->bss[0] && wpa_s->ap_iface->bss[0]) {
        wpa_s->ap_iface->bss[0]->conf->ignore_broadcast_ssid = ssid->ignore_broadcast_ssid;
    }
...
#else //always use 86400 to rekey
        bss->wpa_group_rekey = 86400;
#endif
			     ...
			     struct wpa_ssid *ssid)
{
	...
	if (wpa_s->pairwise_cipher < 0) {
		wpa_printf_warning(MSG_WARNING, "WPA: Failed to select pairwise "
			   "cipher.");
		return -1;
	}
	...
	if (wpa_supplicant_conf_ap(wpa_s, ssid, conf)) {
		wpa_printf_error(MSG_ERROR, "Failed to create AP configuration");
		wpa_supplicant_ap_deinit(wpa_s);
		return -1;
	}
	...
	i = 0;//for (i = 0; i < conf->num_bss; i++) {
		...
		hapd_iface->bss[i]->sta_authorized_cb = NULL;//ap_sta_authorized_cb;
	...
	if (hostapd_setup_interface(wpa_s->ap_iface)) {
		wpa_printf_error(MSG_ERROR, "Failed to initialize AP interface");
		wpa_supplicant_ap_deinit(wpa_s);
		return -1;
	}
...
{
    extern void wpa_ap_auth_receive(struct hostapd_data *hapd, const u8 *sa, const u8 *buf,
			size_t len);
	wpa_ap_auth_receive(wpa_s->ap_iface->bss[0], src_addr, buf, len);
}
