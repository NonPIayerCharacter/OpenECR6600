...
int chip_uart0_pinmux_cfg(int rx_num, int tx_num, int flow_ctrl_en)
{
	if (rx_num == UART0_RX_USED_GPIO5)
	{
		...
		PIN_MUX_SET_REG(AON_PAD_MODE_REG,PIN_MUX_GET_REG(AON_PAD_MODE_REG) & (~(1 << 5)));
	}
	else if (rx_num == UART0_RX_USED_GPIO21)
	{
		PIN_FUNC_SET(IO_MUX_GPIO21, FUNC_GPIO21_UART0_RXD);
		PIN_MUX_SET_REG(AON_PAD_MODE_REG,PIN_MUX_GET_REG(AON_PAD_MODE_REG) & (~(1 << 21)));
	}
	else
	{
		goto uart0_failed;
	...
	if (tx_num == UART0_TX_USED_GPIO6)
	{
		...
		PIN_MUX_SET_REG(AON_PAD_MODE_REG,PIN_MUX_GET_REG(AON_PAD_MODE_REG) & (~(1 << 6)));
	}
	else if (tx_num == UART0_TX_USED_GPIO22)
	{
		PIN_FUNC_SET(IO_MUX_GPIO22, FUNC_GPIO22_UART0_TXD);
		PIN_MUX_SET_REG(AON_PAD_MODE_REG,PIN_MUX_GET_REG(AON_PAD_MODE_REG) & (~(1 << 22)));
	}
	else
	{
		goto uart0_failed;
	...
	if (flow_ctrl_en)
	{
		...
		PIN_MUX_SET_REG(AON_PAD_MODE_REG,PIN_MUX_GET_REG(AON_PAD_MODE_REG) & (~(3 << 3)));
	}
...
void chip_uart1_pinmux_cfg(int flow_ctrl_en)
{
	...
	PIN_MUX_SET_REG(AON_PAD_MODE_REG,PIN_MUX_GET_REG(AON_PAD_MODE_REG) & (~(3 << 1)));
	if (flow_ctrl_en)
	{
		PIN_FUNC_SET(IO_MUX_GPIO24, FUNC_GPIO24_UART1_CTS);
		PIN_FUNC_SET(IO_MUX_GPIO23, FUNC_GPIO23_UART1_RTS);
		PIN_MUX_SET_REG(AON_PAD_MODE_REG,PIN_MUX_GET_REG(AON_PAD_MODE_REG) & (~(3 << 23)));
	}
}
...
int chip_uart2_pinmux_cfg(int tx_num)
{
	...
	if (tx_num == UART2_TX_USED_GPIO0)
	{
		...
		PIN_MUX_SET_REG(AON_PAD_MODE_REG,PIN_MUX_GET_REG(AON_PAD_MODE_REG) & (~(1 << 0)));
	}
...
void chip_pwm_pinmux_cfg(unsigned int cfg)
{
	switch (cfg)
	{
		...
		case PWM_CH1_USED_GPIO1:
			...
			PIN_MUX_SET_REG(AON_PAD_MODE_REG,PIN_MUX_GET_REG(AON_PAD_MODE_REG) & (~(1 << 1)));
			break;

		case PWM_CH1_USED_GPIO23:
			PIN_FUNC_SET(IO_MUX_GPIO23, FUNC_GPIO23_PWM_CTRL1);
			PIN_MUX_SET_REG(AON_PAD_MODE_REG,PIN_MUX_GET_REG(AON_PAD_MODE_REG) & (~(1 << 23)));
			break;

		case PWM_CH2_USED_GPIO2:
			PIN_FUNC_SET(IO_MUX_GPIO2, FUNC_GPIO2_PWM_CTRL2);
			PIN_MUX_SET_REG(AON_PAD_MODE_REG,PIN_MUX_GET_REG(AON_PAD_MODE_REG) & (~(1 << 2)));
			break;

		case PWM_CH2_USED_GPIO24:
			PIN_FUNC_SET(IO_MUX_GPIO24, FUNC_GPIO24_PWM_CTRL2);
			PIN_MUX_SET_REG(AON_PAD_MODE_REG,PIN_MUX_GET_REG(AON_PAD_MODE_REG) & (~(1 << 24)));
			break;
		...
		case PWM_CH2_USED_GPIO16:
			...
			PIN_MUX_SET_REG(AON_PAD_MODE_REG,PIN_MUX_GET_REG(AON_PAD_MODE_REG) & (~(1 << 16)));
			break;
		
		case PWM_CH3_USED_GPIO3:
			PIN_FUNC_SET(IO_MUX_GPIO3, FUNC_GPIO3_PWM_CTRL3);
			PIN_MUX_SET_REG(AON_PAD_MODE_REG,PIN_MUX_GET_REG(AON_PAD_MODE_REG) & (~(1 << 3)));
			break;

		case PWM_CH3_USED_GPIO25:
			PIN_FUNC_SET(IO_MUX_GPIO25, FUNC_GPIO25_PWM_CTRL3);
			PIN_MUX_SET_REG(AON_PAD_MODE_REG,PIN_MUX_GET_REG(AON_PAD_MODE_REG) & (~(1 << 25)));
			break;

		case PWM_CH4_USED_GPIO4:
			PIN_FUNC_SET(IO_MUX_GPIO4, FUNC_GPIO4_PWM_CTRL4);
			PIN_MUX_SET_REG(AON_PAD_MODE_REG,PIN_MUX_GET_REG(AON_PAD_MODE_REG) & (~(1 << 4)));
			break;

		case PWM_CH4_USED_GPIO14:
			PIN_FUNC_SET(IO_MUX_GPIO14, FUNC_GPIO14_PWM_CTRL4);
			PIN_MUX_SET_REG(AON_PAD_MODE_REG,PIN_MUX_GET_REG(AON_PAD_MODE_REG) & (~(1 << 14)));
			break;

		case PWM_CH5_USED_GPIO15:
			PIN_FUNC_SET(IO_MUX_GPIO15, FUNC_GPIO15_PWM_CTRL5);
			PIN_MUX_SET_REG(AON_PAD_MODE_REG,PIN_MUX_GET_REG(AON_PAD_MODE_REG) & (~(1 << 15)));
			break;
...
int chip_i2c_pinmux_cfg(int scl_num, int sda_num)
{
	if (scl_num == I2C_SCL_USED_GPIO18)
	{
		...
		PIN_MUX_SET_REG(AON_PAD_MODE_REG,PIN_MUX_GET_REG(AON_PAD_MODE_REG) & (~(1 << 18)));
	}
...
void chip_io_mode_pinmux_cfg(int io_num,int mode)
{
	if(io_num <= 25)
	{
		PIN_MUX_SET_REG(AON_PAD_MODE_REG,PIN_MUX_GET_REG(AON_PAD_MODE_REG) & (~(1 << io_num)));
	}
	switch (io_num)
	{
		case 0:
