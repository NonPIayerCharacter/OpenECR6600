...
static const T_SPI_FLASH_PARAM kh_flash_params[] = 
{
	{
		.flash_id = KH25L64,
		...
		.flash_otp_block_num  = FLASH_OTP_BLOCK_NUM_2,
		.flash_otp_block_interval = FLASH_OTP_BLOCK_INTERNAL_512_BYTE,
	},
...
static int spiFlash_KH_check_addr(unsigned int flash_id, int addr, int length)
{
	if (flash_id == KH25L64 || flash_id == KH25L128)
	{
		if(!((addr>=0x000000 && addr<=0x0001FF && (addr + length -1) <= 0x0001FF)
		   ||(addr>=0x000200 && addr<=0x0003FF && (addr + length -1) <= 0x0003FF)))
		{
 ...
 */
int spiFlash_KH_OTP_Read_CMD(struct _T_SPI_FLASH_DEV * p_flash_dev, int addr, int length,unsigned char *pdata)
{
	int i,rx_num;
	unsigned int data;	//word aligned
	unsigned int * rxBuf;

	spiFlash_format_addr(0x0B, addr, 
...
int spiFlash_KH_OTP_Read(struct _T_SPI_FLASH_DEV * p_flash_dev, int addr, int length,unsigned char *pdata)
{
	...
	if (spiFlash_KH_check_addr(flash_id, addr, length))
	{
		return FLASH_RET_PARAMETER_ERROR;
	}
	...
	do
	{
		...
		if (spiFlash_KH_OTP_Read_CMD(p_flash_dev, address, leng, pdata+(address - addr)) != FLASH_RET_SUCCESS)
		{
 ...
 */
int spiFlash_KH_OTP_Lock(struct _T_SPI_FLASH_DEV *  p_flash_dev, int LB)
{
	unsigned int irq_flag, data,i;
 ...
 */
int spiFlash_KH_OTP_Se(struct _T_SPI_FLASH_DEV *  p_flash_dev, unsigned int addr)
{
	return FLASH_RET_SUCCESS;
}
 ...
 *     @return      0--ok, -1--spi bus status busy, -2--spi fifo reset not completes, -4--The input parameter addr or length is out of bounds, -7--flash id is not GD series.
 */
static int spiFlash_KH_OTP_Program(struct _T_SPI_FLASH_DEV * p_flash_dev, unsigned int addr, unsigned char * buf, unsigned int length)
{
	...
	spiFlash_format_addr(0x02, addr, 
 ...
 */
int spiFlash_KH_OTP_Write(struct _T_SPI_FLASH_DEV *  p_flash_dev, unsigned int addr, unsigned int len, unsigned char * buf)
{
	unsigned int address, length;
	unsigned int flash_id = p_flash_dev->flash_param.flash_id;

	if (spiFlash_KH_check_addr(flash_id, addr, len))
	{
		return FLASH_RET_PARAMETER_ERROR;
	}
	...
	do
	{
		...
		if (spiFlash_KH_OTP_Program(p_flash_dev, address, buf+(address - addr), length) != FLASH_RET_SUCCESS)
		{
			system_irq_restore(irq_flag);
...
int spiFlash_KH_probe(T_SPI_FLASH_DEV *p_flash_dev)
{
	...
	for (i=0; i<FLASH_ARRAY_SIZE(kh_flash_params); i++)
	{
		param = &kh_flash_params[i];
	...
	if (i == FLASH_ARRAY_SIZE(kh_flash_params))
	{
		return FLASH_RET_ID_ERROR;
	}

	ops = &p_flash_dev->flash_otp_ops;
	ops->flash_otp_read = spiFlash_KH_OTP_Read;
	ops->flash_otp_write = spiFlash_KH_OTP_Write;
	ops->flash_otp_erase= spiFlash_KH_OTP_Se;
	ops->flash_otp_lock = spiFlash_KH_OTP_Lock;
