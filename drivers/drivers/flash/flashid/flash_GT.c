...
/* GD */
#define GT25Q16A			0x1560C4
#define GT25Q32A			0x1660C4



static const T_SPI_FLASH_PARAM gd_flash_params[] = 
{
	{
		.flash_id = GT25Q16A,
...

	{
		...
		.flash_otp_block_interval = FLASH_OTP_BLOCK_INTERNAL_1024_BYTE,
	},
};


static int spiFlash_GT_check_addr(unsigned int flash_id, int addr, int length)
{
	if (flash_id == GT25Q16A )
	{
	...
	else if(flash_id == GT25Q32A )
	{
		if(!((addr>=0x000000 && addr<=0x0003FF && (addr + length - 1) <= 0x0003FF)
		    ||(addr>=0x000400 && addr<=0x0007FF && (addr + length - 1) <= 0x0007FF)
		    ||(addr>=0x000800 && addr<=0x000bFF && (addr + length - 1) <= 0x000bFF)))
		{
 ...
 */
int spiFlash_GT_OTP_Read_CMD(struct _T_SPI_FLASH_DEV * p_flash_dev, int addr, int length,unsigned char *pdata)
{
	int i,rx_num;
...
int spiFlash_GT_OTP_Read(struct _T_SPI_FLASH_DEV * p_flash_dev, int addr, int length,unsigned char *pdata)
{
	...
	if (spiFlash_GT_check_addr(flash_id, addr, length))
	{
		return FLASH_RET_PARAMETER_ERROR;
	}
	...
	//os_printf(LM_CMD,LL_ERR,"read addr = 0x%x, length = 0x%x\n", addr,length);
	...
	do
	{
		...
		if (spiFlash_GT_OTP_Read_CMD(p_flash_dev, address, leng, pdata+(address - addr)) != FLASH_RET_SUCCESS)
		{
 ...
 */
int spiFlash_GT_OTP_Lock(struct _T_SPI_FLASH_DEV *  p_flash_dev, int LB)
{
	...
	else if(flash_id == GT25Q32A)
	{
		...
		switch (LB)
		{
			...
			case 0x0F:				
				...
				break;
			default:
				//os_printf(LM_OS, LL_INFO, "parameter error\n");
				return FLASH_RET_PARAMETER_ERROR;
		}
		
		irq_flag = system_irq_save();
		spiFlash_cmd_wrSR_2(data);
		system_irq_restore(irq_flag);
		return FLASH_RET_SUCCESS;
	}
	else
 ...
 */
int spiFlash_GT_OTP_Se(struct _T_SPI_FLASH_DEV *  p_flash_dev, unsigned int addr)
{
	...
	else if(flash_id == GT25Q32A)
	{
		...
		if (!((addr>=0x000000 && addr<=0x0003FF) || (addr>=0x000400 && addr<=0x0007FF) || (addr>=0x000800 && addr<=0x000bFF)))
		{
			//cli_printf("addr or length error\n");
			return FLASH_RET_PARAMETER_ERROR;
		}
 ...
 *     @return      0--ok, -1--spi bus status busy, -2--spi fifo reset not completes, -4--The input parameter addr or length is out of bounds, -7--flash id is not GD series.
 */
static int spiFlash_GT_OTP_Program(struct _T_SPI_FLASH_DEV * p_flash_dev, unsigned int addr, unsigned char * buf, unsigned int length)
{
 ...
 */
int spiFlash_GT_OTP_Write(struct _T_SPI_FLASH_DEV *  p_flash_dev, unsigned int addr, unsigned int len, unsigned char * buf)
{
	unsigned int address, length;
	unsigned int flash_id = p_flash_dev->flash_param.flash_id;

	if (spiFlash_GT_check_addr(flash_id, addr, len))
	{
		return FLASH_RET_PARAMETER_ERROR;
	}
	...
	//os_printf(LM_CMD,LL_ERR,"write addr = 0x%x, length = 0x%x\n", addr,length);
	do
	{
		unsigned int j, data;
		unsigned int irq_flag = system_irq_save();
		
		if (spiFlash_GT_OTP_Program(p_flash_dev, address, buf+(address - addr), length) != FLASH_RET_SUCCESS)
		{
			system_irq_restore(irq_flag);
...
int spiFlash_GT_probe(T_SPI_FLASH_DEV *p_flash_dev)
{
	...
	for (i=0; i<FLASH_ARRAY_SIZE(gd_flash_params); i++)
	{
		param = &gd_flash_params[i];
	...
	if (i == FLASH_ARRAY_SIZE(gd_flash_params))
	{
		return FLASH_RET_ID_ERROR;
	}

	ops = &p_flash_dev->flash_otp_ops;
	ops->flash_otp_read = spiFlash_GT_OTP_Read;
	ops->flash_otp_write = spiFlash_GT_OTP_Write;
	ops->flash_otp_erase= spiFlash_GT_OTP_Se;
	ops->flash_otp_lock = spiFlash_GT_OTP_Lock;

	return FLASH_RET_SUCCESS;
}


#endif//end CONFIG_ID_GT25Q16A
