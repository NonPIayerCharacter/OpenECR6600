 ...
 *     @return      0--ok, -7--flash id is not FM series.
 */
...
static const T_SPI_FLASH_PARAM fm_flash_params[] = 
{
	{
		.flash_id = FM25QH16B,
		.flash_size = FLASH_SIZE_2_MB,
		.flash_name = "FM25QH16B",
...
static int spiFlash_FM_check_addr(unsigned int flash_id, int addr, int length)
{
	if (flash_id == FM25QH16B)
	{
 ...
 */
int spiFlash_FM_OTP_Read_CMD(struct _T_SPI_FLASH_DEV * p_flash_dev, int addr, int length,unsigned char *pdata)
{
	int i,rx_num;
...
int spiFlash_FM_OTP_Read(struct _T_SPI_FLASH_DEV * p_flash_dev, int addr, int length,unsigned char *pdata)
{
	...
	if (spiFlash_FM_check_addr(flash_id, addr, length))
	{
		return FLASH_RET_PARAMETER_ERROR;
	}
	...
	do
	{
		...
		if (spiFlash_FM_OTP_Read_CMD(p_flash_dev, address, leng, pdata+(address - addr)) != FLASH_RET_SUCCESS)
		{
 ...
 */
int spiFlash_FM_OTP_Lock(struct _T_SPI_FLASH_DEV *  p_flash_dev, int LB)
{
 ...
 */
int spiFlash_FM_OTP_Se(struct _T_SPI_FLASH_DEV *  p_flash_dev, unsigned int addr)
{
 ...
 *     @return      0--ok, -1--spi bus status busy, -2--spi fifo reset not completes, -4--The input parameter addr or length is out of bounds, -7--flash id is not GD series.
 */
static int spiFlash_FM_OTP_Program(struct _T_SPI_FLASH_DEV * p_flash_dev, unsigned int addr, unsigned char * buf, unsigned int length)
{
 ...
 */
int spiFlash_FM_OTP_Write(struct _T_SPI_FLASH_DEV *  p_flash_dev, unsigned int addr, unsigned int len, unsigned char * buf)
{
	unsigned int address, length;
	unsigned int flash_id = p_flash_dev->flash_param.flash_id;

	if (spiFlash_FM_check_addr(flash_id, addr, len))
	{
		return FLASH_RET_PARAMETER_ERROR;
	}
	...
	do
	{
		...
		if (spiFlash_FM_OTP_Program(p_flash_dev, address, buf+(address - addr), length) != FLASH_RET_SUCCESS)
		{
			system_irq_restore(irq_flag);
...
int spiFlash_FM_probe(T_SPI_FLASH_DEV *p_flash_dev)
{
	...
	for (i=0; i<FLASH_ARRAY_SIZE(fm_flash_params); i++)
	{
		param = &fm_flash_params[i];
	...
	if (i == FLASH_ARRAY_SIZE(fm_flash_params))
	{
		return FLASH_RET_ID_ERROR;
	}

	ops = &p_flash_dev->flash_otp_ops;
	ops->flash_otp_read = spiFlash_FM_OTP_Read;
	ops->flash_otp_write = spiFlash_FM_OTP_Write;
	ops->flash_otp_erase= spiFlash_FM_OTP_Se;
	ops->flash_otp_lock = spiFlash_FM_OTP_Lock;

	return FLASH_RET_SUCCESS;
}


#endif//end CONFIG_ID_FM25QH16B
