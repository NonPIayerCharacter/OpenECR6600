...
static const T_SPI_FLASH_PARAM puya_flash_params[] = 
{
	{
		.flash_id = P25Q80HD,
...
//	},
	...
	{
		.flash_id = P25Q16SH_OLD,
		.flash_size = FLASH_SIZE_2_MB,
		.flash_name = "P25Q16SH_OLD",
		.flash_otp_star_addr  = FLASH_OTP_START_ADDR_0x1000,
		.flash_otp_block_size = FLASH_OTP_BLOCK_SIZE_1024_BYTE,
		.flash_otp_block_num  = FLASH_OTP_BLOCK_NUM_3,
		.flash_otp_block_interval = FLASH_OTP_BLOCK_INTERNAL_4096_BYTE,
...
static int spiFlash_PUYA_check_addr(unsigned int flash_id, int addr, int length)
{
	unsigned int data;
	
	if(flash_id == P25Q80HD)
	{
		...
		|| (addr>=0x003000 && addr<=0x0031FF && (addr + length - 1) > 0x0030FF))
		{
			...
			data = spiFlash_format_rd(SPIFLASH_CMD_RDSR_CFG,1);
			spiFlash_format_wr(SPIFLASH_CMD_WRSR_2, (data | BIT7), 1);//write DP bit
 ...
 */
int spiFlash_PUYA_OTP_Read_CMD(struct _T_SPI_FLASH_DEV * p_flash_dev, int addr, int length,unsigned char *pdata)
{
	int i,rx_num;
...
int spiFlash_PUYA_OTP_Read(struct _T_SPI_FLASH_DEV * p_flash_dev, int addr, int length,unsigned char *pdata)
{
	...
	if (spiFlash_PUYA_check_addr(flash_id, addr, length))
	{
		return FLASH_RET_PARAMETER_ERROR;
	}
	...
	do
	{
		...
		if (spiFlash_PUYA_OTP_Read_CMD(p_flash_dev, address, leng, pdata+(address - addr)) != FLASH_RET_SUCCESS)
		{
			system_irq_restore(irq_flag);
 ...
 */
int spiFlash_PUYA_OTP_Lock(struct _T_SPI_FLASH_DEV *  p_flash_dev, int LB)
{
 ...
 */
int spiFlash_PUYA_OTP_Se(struct _T_SPI_FLASH_DEV *  p_flash_dev, unsigned int addr)
{
	...
	if(flash_id == P25Q80HD)
	{
		...
		{
			//os_printf(LM_OS, LL_INFO, "addr error\n");
			return FLASH_RET_PARAMETER_ERROR;
		}
	}
    else if(flash_id == P25Q16SH || flash_id == P25Q16SH_OLD || flash_id == P25Q32SH || flash_id == P25Q16HB) 
	{
		...
		{
			os_printf(LM_OS, LL_ERR, "addr error\n");
			return FLASH_RET_PARAMETER_ERROR;
		}
	}
	else
 ...
 *     @return      0--ok, -1--spi bus status busy, -2--spi fifo reset not completes, -4--The input parameter addr or length is out of bounds, -7--flash id is not GD series.
 */
static int spiFlash_PUYA_OTP_Program(struct _T_SPI_FLASH_DEV * p_flash_dev, unsigned int addr, unsigned char * buf, unsigned int length)
{
 ...
 */
int spiFlash_PUYA_OTP_Write(struct _T_SPI_FLASH_DEV *  p_flash_dev, unsigned int addr, unsigned int len, unsigned char * buf)
{
	unsigned int address, length;
	
	unsigned int flash_id = p_flash_dev->flash_param.flash_id;

	if (spiFlash_PUYA_check_addr(flash_id, addr, len))
	{
		return FLASH_RET_PARAMETER_ERROR;
	}
	...
	do
	{
		...
		if (spiFlash_PUYA_OTP_Program(p_flash_dev, address, buf+(address - addr), length) != FLASH_RET_SUCCESS)
		{
			system_irq_restore(irq_flag);
...
int spiFlash_PUYA_probe(T_SPI_FLASH_DEV *p_flash_dev)
{
	...
	for (i=0; i<FLASH_ARRAY_SIZE(puya_flash_params); i++)
	{
		param = &puya_flash_params[i];
	...
	if (i == FLASH_ARRAY_SIZE(puya_flash_params))
	{
		return FLASH_RET_ID_ERROR;
	}

	ops = &p_flash_dev->flash_otp_ops;
	ops->flash_otp_read = spiFlash_PUYA_OTP_Read;
	ops->flash_otp_write = spiFlash_PUYA_OTP_Write;
	ops->flash_otp_erase= spiFlash_PUYA_OTP_Se;
	ops->flash_otp_lock = spiFlash_PUYA_OTP_Lock;
